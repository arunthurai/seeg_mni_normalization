...
Install from github with pip::
    ...
    pip install -e git+https://github.com/arunthurai/seeg_mni_normalization#egg=seeg_mni_normalization
...
Do a dry-run first (``-n``) and simply print (``-p``) what would be run::
    ...
    seeg_mni_normalization /path/to/bids/dir /path/to/output/dir participant -np

Run the app, using all cores::
    
    seeg_mni_normalization /path/to/bids/dir /path/to/output/dir participant --cores all
...
To generate a report, run::
    ...
    seeg_mni_normalization /path/to/bids/dir /path/to/output/dir participant --report
...
Here are some instructions to get your python environment set-up on graham to run seeg_mni_normalization:

#. Create a virtualenv and activate it::

      cd $SCRATCH
      module load python/3
      virtualenv venv_seeg_mni_normalization
      source venv_seeg_mni_normalization/bin/activate
...
In an interactive job (for testing)::
    ...
    seeg_mni_normalization bids_dir out_dir participant --participant_label 001 -j 8


Submitting a job (for larger cores, more subjects), still single job, but snakemake will parallelize over the 32 cores::

    regularSubmit -j Fat seeg_mni_normalization bids_dir out_dir participant  -j 32


Scaling up to ~hundred subjects (needs cc-slurm snakemake profile installed), submits 1 16core job per subject::
    
    seeg_mni_normalization bids_dir out_dir participant  --profile cc-slurm
...
Scaling up to even more subjects (uses group-components to bundle multiple subjects in each job), 1 32core job for N subjects (e.g. 10)::
    ...
    seeg_mni_normalization bids_dir out_dir participant  --profile cc-slurm --group-components subj=10
